<?php

return [
    'loading' => [
        'title' => 'Getting your quotes…',
        'subtitle' => "We're fetching live prices for you. This usually takes less than 30 seconds.",
    ],

    'nav_label' => 'Auto Insurance',

    'companies' => [
        // The dedicated insurance listing - see
        // organizations/insurance-companies.blade.php.
        'stat_verified' => 'Verified insurers',
        'stat_verified_sub' => 'Trusted and regulated',
        'stat_reviews' => 'Real customer reviews',
        'stat_reviews_sub' => 'Make confident choices',
        'sort_by' => 'Sort by',
        'sort_rated' => 'Top rated',
        'sort_reviewed' => 'Most reviewed',
        'sort_name' => 'Name A-Z',
        'select_to_compare' => 'Select companies to compare',
        'view_profile' => 'View profile',
        'get_quote' => 'Get quote',
        'branches' => '{0} No branches listed|{1} 1 branch|[2,*] :count branches',
        'helps_title' => 'How Findex helps',
        'helps_1' => 'Compare all insurers in one place',
        'helps_2' => 'Read real customer reviews',
        'helps_3' => 'Check coverage options and prices',
        'helps_4' => 'Request quotes quickly',
        'helps_5' => 'Make a confident choice',
        'quote_card_title' => 'Get an auto insurance quote',
        'quote_card_body' => 'Compare offers from every insurer we track, in minutes.',
        'heading' => 'Insurance Companies in Armenia',
        'subtitle' => 'Every insurance company Findex tracks, in one place. Read reviews from real customers before you request a quote.',
        'stat_count' => 'Companies listed',
        'cta_quote' => 'Get an Auto Insurance Quote',
    ],

    'request' => [
        'heading' => 'Compare Auto Insurance Quotes',
        'subheading' => "Enter your vehicle's plate number once, and we'll fetch live quotes from our partner insurers - compare them side by side in seconds.",
        'badge' => 'Partner Insurers',
        'step_1_title' => 'Tell us once',
        'step_1_body' => 'Plate, owner ID and contract term',
        'step_2_title' => 'We ask insurers',
        'step_2_body' => 'We request live quotes from partner insurers',
        'step_3_title' => 'You compare instantly',
        'step_3_body' => 'Real quotes on one page, cheapest first',
        'section_vehicle' => 'Vehicle & owner',
        'section_details' => 'Your details',
        'vehicle_plate' => 'Vehicle plate number',
        'vehicle_plate_placeholder' => 'e.g. 01AA123',
        'owner_types' => [
            'individual' => 'Individual',
            'legal_entity' => 'Legal entity',
        ],
        'owner_id_number' => "Owner's Passport/ID/PSC",
        'contract_term' => 'Contract term',
        'contract_terms' => [
            3 => '3 months',
            6 => '6 months',
            12 => '12 months',
        ],
        'market_heading' => 'Contact and payout details',
        'market_explainer' => 'Insurers quote through the Motor Insurers Bureau, which needs a phone number, an email and a bank account before it returns a price. These are sent only with this quote request and are never stored by Findex.',
        'market_phone' => 'Phone number',
        'market_email' => 'Email address',
        'market_bank_account' => 'Bank account number',
        'market_bank_account_hint' => 'Required by the insurer, sent only with that quote request, and never stored by Findex.',
        'consent' => "I agree that these vehicle details will be shared with Findex's partner insurers so they can prepare a quote.",
        'submit' => 'Get quotes',
        'data_secure_note' => 'Sent securely to fetch quotes. Never stored by Findex.',
        'aside_secure_title' => 'Secure & private',
        'aside_secure_body' => 'Your details go straight to the insurers to fetch a price. Findex never stores your ID or bank account.',
        'aside_next_title' => 'What happens next?',
        'aside_next_1' => 'We ask every partner insurer for a live price.',
        'aside_next_2' => 'Quotes land on one page, cheapest first.',
        'aside_next_3' => 'Pick an insurer and go straight to them.',
        'why_title' => 'Why use Findex?',
        'why_1_title' => 'Save time',
        'why_1_body' => 'One request, many quotes.',
        'why_2_title' => 'Best prices',
        'why_2_body' => 'Compare and choose the best deal.',
        'why_3_title' => 'Trusted partners',
        'why_3_body' => 'We work only with reliable insurers.',
    ],

    'results' => [
        // The offers toolbar, the sidebar cards beside the list, and the
        // compare tray - see insurance/auto/show.blade.php.
        'sort_by' => 'Sort by',
        'sort_price_asc' => 'Lowest price first',
        'sort_price_desc' => 'Highest price first',
        'sort_rating' => 'Best rated first',
        'about_insurer' => 'About insurer',
        'no_reviews' => 'No reviews yet',
        'reviews_count' => ':count reviews',
        'price_overview' => 'Price overview',
        'price_lowest' => 'Lowest price',
        'price_average' => 'Average price',
        'price_highest' => 'Highest price',
        'price_saving' => 'You can save up to :amount by choosing the lowest offer.',
        'coverage_includes' => 'What TPL covers',
        'coverage_note' => 'Compulsory motor TPL is set by law, so cover is the same whichever insurer you pick - only the price and the service differ.',
        'coverage_item_liability' => 'Third-party liability (TPL)',
        'coverage_item_property' => 'Damage to other people\'s property',
        'coverage_item_injury' => 'Personal injury to third parties',
        'coverage_item_area' => 'Valid throughout Armenia',
        'coverage_excluded' => 'It does not cover damage to your own vehicle.',
        'companies_title' => 'About the insurers',
        'companies_body' => 'Read reviews and see what else each company covers before you decide.',
        'companies_cta' => 'Browse insurers',
        'interested_badge' => 'Interest sent',
        // The redesigned results screen: the hero's reassurance row, the
        // "your details" bar under it, and the sidebar beside the offers.
        'benefit_real' => 'Real offers from trusted insurers',
        'benefit_no_fees' => 'No hidden fees',
        'benefit_save' => 'Save time and money',
        'your_details' => 'Your details',
        'vehicle_label' => 'Vehicle',
        'coverage_type_label' => 'Coverage type',
        'view_details' => 'View details',
        'hide_details' => 'Hide details',
        'tips_title' => 'Useful tips',
        'tip_mandatory' => 'TPL is mandatory for all vehicles in Armenia.',
        'tip_compare' => 'Compare coverage and additional services, not just price.',
        'tip_network' => 'Check the insurer\'s service network near you.',
        'tip_online' => 'Buying online is usually quicker than visiting a branch.',
        'heading' => 'Your Auto Insurance Quotes',
        'ready_heading' => 'Your insurance quotes are ready',
        'ready_subtitle' => 'We found :count offers for your vehicle. Compare prices and choose what works best.',
        'quotes_received' => ':count quotes received',
        'trail_vehicle' => 'Vehicle',
        'trail_details' => 'Details',
        'trail_compare' => 'Compare',
        'product_name' => 'Compulsory motor TPL',
        'total_for' => 'Total for :term',
        'edit_details' => 'Edit details',
        'sorted_by_price' => 'Lowest price first',
        'sort_label' => 'Sort by',
        'sort_price' => 'Lowest price',
        'offers_heading' => ':count insurance offers',
        'empty_heading' => "We couldn't find an online quote",
        'empty_body' => 'No participating insurer returned a price for these details. Review your information and try again.',
        'empty_retry' => 'Review details and try again',
        'submitted' => 'Done! :count partner insurers responded - compare their quotes below.',
        'vehicle_summary' => 'Plate :plate · :term',
        'visit_site' => 'Visit :insurer',
        'best_price_badge' => 'Best price',
        'premium_label' => 'Premium',
        'term_label' => 'Policy term',
        'term_months' => ':months months',
        'coverage_summary_label' => "What's covered",
        'notes_label' => 'Notes',
        'declined_label' => 'Not available',
        'declined_hint' => "This insurer isn't able to cover this vehicle.",
        'no_quotes_yet' => "We don't have a partner insurer set up yet - check back soon.",
        'compare_hint' => 'Got more than one quote? Select a few to line them up side by side.',
        'add_to_compare' => 'Select to compare',
        'compare_bar_button' => 'Compare',
        'compare_bar_clear' => 'Clear',
        'quotes_selected' => 'quotes selected',
        'compare_max_reached' => 'You can compare up to 3 quotes at a time.',
        'compare_heading' => 'Side by Side',
        'compare_row_premium' => 'Premium',
        'compare_row_coverage' => "What's covered",
        'compare_row_notes' => 'Notes',
        'bookmark_hint' => 'Bookmark this page to come back to your quotes any time.',
        'interested_button' => "I'm interested",
        'interested_confirmation' => "You've let this insurer know you're interested - expect them to reach out.",
        'interest_marked_status' => "Got it - we've let the insurer know you're interested.",
    ],

    'provider' => [
        'coverage_summaries' => [
            'Compulsory third-party liability - covers damage and injury you cause to other people or their property.',
            'Full compulsory third-party liability coverage, including free accident assessment at any of our branches.',
            'Compulsory third-party liability coverage with fast, fully digital claims processing.',
            'Compulsory third-party liability coverage backed by a nationwide branch and service network.',
        ],
        'quote_notes' => [
            'Includes 24/7 roadside assistance. Final price may vary after a vehicle inspection.',
            'Free towing up to 50 km and a replacement car for up to 3 days while your vehicle is repaired.',
            'Instant e-policy issuance - drive away with proof of insurance in minutes.',
            'Dedicated claims manager and a mobile app to track your policy and file claims.',
        ],
    ],

    'email' => [
        'interest_subject' => 'A customer is interested in your quote',
        'interest_greeting' => 'Hi :organization team,',
        'interest_heading' => 'New interested customer',
        'interest_body' => ':name is interested in your :premium quote for vehicle :plate.',
        'interest_contact' => 'Reach out to them at :email.',
    ],
];
