...
@section('title', __('writer.login_title') . ' — Findex')

@section('content')
    <section class="mx-auto max-w-md px-6 py-16 lg:py-24">
        <h1 class="font-heading text-2xl font-bold text-ink">{{ __('writer.login_title') }}</h1>

        <form method="POST" action="{{ route('writer.login') }}" class="mt-8 space-y-5" novalidate>
            @csrf
        ...
        <p class="mt-6 text-center text-xs text-subtle">
            {{ __('writer.personal_login_prompt') }}
            <a href="{{ route('login') }}" class="font-medium text-primary hover:underline">{{ __('writer.personal_login_link') }}</a>
        </p>
    </section>
@endsection
