@php
    ...
    $bankingColumns = array_values(
        array_filter(
            [
                array_values(
                    array_filter([
                        $section('nav.banking.groups.loans', [
                            'mortgages',
                            'personal-loans',
                            'business-loans',
                            'student-loans',
...
<header x-data="{ mobileOpen: false }" @class([
        ...
        <nav class="hidden flex-wrap items-center gap-x-5 gap-y-2 text-sm text-ink lg:flex">
            @foreach ($navItems as $navItem)
                @if (empty($navItem['columns']))
                    <a href="{{ $navItem['href'] }}"
                        class="flex items-center gap-1.5 whitespace-nowrap hover:text-primary {{ $navItem['active'] ? 'text-primary' : '' }}">
                    ...
                    <div x-data="{ open: false }" class="relative" @click.outside="open = false">
                        <button type="button" @click="open = !open"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 8"
                                class="h-2 w-3 fill-none stroke-current" :class="{ 'rotate-180': open }">
                                <path d="M1 1.5 6 6.5 11 1.5" stroke-width="1.5" stroke-linecap="round"
                        ...
                        <div x-show="open" x-transition x-cloak
                            ...
                            <div
                                class="grid gap-x-8 {{ count($dropdown['columns']) > 1 ? 'grid-cols-2' : 'grid-cols-1' }}">
                                @foreach ($dropdown['columns'] as $column)
                                    <div class="space-y-5">
                                        @foreach ($column as $section)
                                            <div>
                                                <p
                                                    class="px-2 pb-1 text-[10px] font-semibold tracking-wider text-subtle uppercase">
                                                    {{ $section['heading'] }}
                                                </p>
                                                @foreach ($section['items'] as $item)
                                                    <a href="{{ $item['href'] }}"
                                                        class="flex items-start justify-between gap-3 rounded-lg px-2 py-2 text-sm leading-snug text-body-text transition hover:bg-primary/5 hover:text-primary">
                                                        {{ $item['label'] }}
                                                    </a>
                                                @endforeach
                                            </div>
        ...
        <div class="flex items-center gap-5">
            @if ($connectLinks->isNotEmpty())
                <div x-data="{ open: false }" class="relative hidden sm:block" @click.outside="open = false">
                    <button type="button" @click="open = !open" aria-label="{{ __('nav.connect') }}"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                            class="h-4 w-4 fill-none stroke-current xl:hidden">
                            <path
                                d="M15 17h5l-1.4-1.4a2 2 0 0 1-.6-1.4V11a6 6 0 0 0-5-5.9V4a1 1 0 1 0-2 0v1.1A6 6 0 0 0 6 11v3.2a2 2 0 0 1-.6 1.4L4 17h5"
                                stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" />
                            <path d="M9 17a3 3 0 0 0 6 0" stroke-width="1.6" stroke-linecap="round"
                                stroke-linejoin="round" />
                        </svg>
                    ...
                    <div x-show="open" x-transition x-cloak
                        ...
                        @foreach ($connectLinks as $link)
                            <a href="{{ $link['url'] }}"
                                ...
                                @if ($link['connected'])
                                    <span
                                        class="shrink-0 text-xs font-medium text-primary">{{ __('nav.connected') }}</span>
                                @endif
                            </a>
                        @endforeach
                    </div>
            ...
            <div x-data="{ open: false }" class="relative hidden sm:block" @click.outside="open = false">
                <button type="button" @click="open = !open"
                    class="flex items-center gap-1.5 text-sm text-ink hover:text-primary" :aria-expanded="open"
                    aria-label="Language">
                    <span
                        class="text-base leading-none">{{ config('localization.available')[app()->getLocale()]['flag'] ?? '🌐' }}</span>
                    <span
                        class="sr-only">{{ config('localization.available')[app()->getLocale()]['native'] ?? strtoupper(app()->getLocale()) }}</span>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 8" class="h-2 w-3 fill-none stroke-current"
                ...
                <div x-show="open" x-transition x-cloak
                    ...
                    @foreach (config('localization.available') as $code => $locale)
                        <a href="{{ route($currentRoute, array_merge($currentRouteParams, ['locale' => $code])) }}"
            ...
            @auth
                <div x-data="{ open: false }" class="relative hidden sm:block" @click.outside="open = false">
                    <button type="button" @click="open = !open"
                        class="flex items-center gap-2 text-sm text-ink hover:text-primary" :aria-expanded="open">
                    ...
                    <div x-show="open" x-transition x-cloak
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="block w-full px-4 py-2 text-left text-sm text-body-text hover:bg-primary/5 hover:text-primary">
                                {{ __('common.logout') }}
                            </button>
                        </form>
                    </div>
            ...
            <button type="button" @click="mobileOpen = !mobileOpen" class="text-ink lg:hidden"
                aria-label="{{ __('common.menu') }}" :aria-expanded="mobileOpen">
                <svg x-show="!mobileOpen" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                    class="h-6 w-6 fill-none stroke-current">
                    <path d="M4 6h16M4 12h16M4 18h16" stroke-width="1.6" stroke-linecap="round" />
                </svg>
                <svg x-show="mobileOpen" x-cloak xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                    class="h-6 w-6 fill-none stroke-current">
    ...
    <div x-show="mobileOpen" x-cloak x-transition @keydown.escape.window="mobileOpen = false" role="dialog"
        aria-modal="true" class="fixed inset-0 z-50 overflow-y-auto bg-white lg:hidden">
        <div class="flex items-center justify-between border-b border-placeholder px-6 py-5">
            ...
            <button type="button" @click="mobileOpen = false" class="text-ink"
        ...
        <div class="px-6 py-4">
            <nav class="flex flex-col gap-1 text-sm text-ink">
                <a href="{{ route('home') }}"
                    class="rounded-md px-2 py-2 hover:bg-primary/5 hover:text-primary">{{ __('nav.home') }}</a>

                @foreach ($navItems as $navItem)
                    @if (empty($navItem['columns']))
                        <a href="{{ $navItem['href'] }}"
                            class="flex items-center gap-2 rounded-md px-2 py-2 hover:bg-primary/5 hover:text-primary {{ $navItem['active'] ? 'text-primary' : '' }}">
                            <x-nav-icon :name="$navItem['icon']" />
                            {{ $navItem['label'] }}
                        </a>
                    @else
                        ...
                        <div x-data="{ open: false }">
                            <button type="button" @click="open = !open"
                                class="flex w-full items-center justify-between rounded-md px-2 py-2 hover:bg-primary/5 hover:text-primary {{ $dropdown['active'] ? 'text-primary' : '' }}">
                            ...
                            <div x-show="open" x-cloak
                                class="ml-4 flex flex-col gap-4 border-l border-placeholder pl-4">
                                @foreach ($dropdown['columns'] as $column)
                                    @foreach ($column as $section)
                                        <div class="flex flex-col gap-1">
                                            <p
                                                class="px-3 text-[10px] font-semibold tracking-wider text-subtle uppercase">
                                                {{ $section['heading'] }}
                                            </p>
                                            @foreach ($section['items'] as $item)
                                                <a href="{{ $item['href'] }}"
                                                    class="flex items-start justify-between gap-3 rounded-lg px-3 py-2.5 leading-snug text-body-text hover:bg-primary/5 hover:text-primary">
                                                    {{ $item['label'] }}
                                                </a>
                                            @endforeach
                                        </div>
                ...
                @if ($connectLinks->isNotEmpty())
                    <div class="mt-1 border-t border-placeholder pt-3">
                        <p class="px-2 pb-1 text-xs font-semibold tracking-wider text-subtle uppercase">
                            {{ __('nav.connect') }}</p>
                        @foreach ($connectLinks as $link)
                            <a href="{{ $link['url'] }}"
